...
public class SkullKnight extends Enemy {
    ...
    private int customHealth = 55; // Set health to 55 for this miniboss
    ...
    public SkullKnight(Point startLocation, Point endLocation, Direction facingDirection) {
        super(startLocation.x, startLocation.y, new SpriteSheet(ImageLoader.load("skullknight.png"), 14, 17), "WALK_RIGHT");
    ...
    public void update(Player player) {
        ...
        if (dinosaurState == DinosaurState.SHOOT) {
            shootLargeFireball(); // Shoot a large fireball
    ...
    private void shootLargeFireball() {
        int fireballX;
        float fireballSpeed;
        if (facingDirection == Direction.RIGHT) {
            fireballX = Math.round(getX()) + getWidth();
            fireballSpeed = 2.0f; // Adjust speed as needed
        } else {
        ...
        LargeFireball fireball = new LargeFireball(new Point(fireballX, fireballY), fireballSpeed, 100); // Adjust existence frames as needed
    ...
    public void draw(GraphicsHandler graphicsHandler) {
        super.draw(graphicsHandler);  // Call super to draw the miniboss
        drawHealthBar(graphicsHandler);  // Draw the health bar above the miniboss
    }
